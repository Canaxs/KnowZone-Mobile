...
export default function TabLayout() {
  ...
  useEffect(() => {
    if (user?.id) {
      backgroundLocationService.start(user.id);
      console.log('Background location service started from tab layout');
    }

    return () => {
      // Tab layout'tan çıkıldığında da çalışmaya devam et
      // Burada stop() çağırmayın
    };
  }, [user?.id]);
